...
<Note>
  ...
  12+, Linux (Ubuntu 20.04+, Debian 11+, or Fedora 38+), or Windows 10+ with WSL2, plus admin or sudo
  access.
</Note>
...
created — go to `/login` and use those credentials instead.
  ...
  <Step title="Choose the first-proof path">
    When `/api/config/platform` lists `docker` as an enabled deploy target, click **Launch local Docker demo — no API key** on **Getting Started**. Nora enables its built-in deterministic provider and deploys a demo agent in one action. If the page reports that the local Docker demo is unavailable, add a real provider and deploy to one of the targets enabled by your operator.
