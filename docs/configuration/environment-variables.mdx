...
| `SIGNUP_BOT_PROTECTION_PROVIDER`    | No       | `none`     | Runtime challenge provider: `none`, `turnstile`, or `recaptcha`. The backend publishes only safe public metadata. |
...
| `PROXMOX_OFFLINE_STAGE_COMMAND`            | Non-root SSH only   | —                                                          | Absolute path to an operator-installed strict helper that stages managed state into a stopped LXC. Generic sudo or shell commands are rejected. |
