...
<Warning>
  ...
  operations. Existing records remain visible, and direct owners can still use **Stop** or
  **Delete** to clean up an already-running remote workload, but other operations require switching
  back to `selfhosted`. Docker, Kubernetes, and Proxmox agent records are otherwise retained; the
  new mode controls which future operations and deployment limits are accepted.
</Warning>
