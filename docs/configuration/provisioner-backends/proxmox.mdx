...
Root SSH is the supported direct path for stopped-LXC reconciliation. A non-root SSH account needs a separately installed, narrowly scoped helper configured through `PROXMOX_OFFLINE_STAGE_COMMAND`. The value must be one absolute executable path; Nora rejects generic `sudo sh -c`, arguments, and arbitrary shell prefixes. `PROXMOX_SUDO` still applies to ordinary fixed `pct` lifecycle commands, but it is not sufficient for mounting and safely editing a stopped root filesystem. If neither root SSH nor the strict helper is available, start and restart fail closed before the LXC can run stale credentials.
