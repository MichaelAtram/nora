...
Remote-host administration is intentionally session-only. Register and manually test the host in
**App -> Remote Hosts** before using its `remote:<id>` target. Workspace API keys cannot call any
`/remote-hosts` endpoint, and even session-authenticated responses mask stored SSH private keys,
...
The session user must own the host or have an `editor`, `admin`, or `owner` role in a workspace where
the host is shared. The created agent belongs to the deploying user and is not automatically assigned
to that workspace.

`POST /agents/deploy` also accepts a workspace API key with `agents:write`, but a remote-host access
check uses the key's issuing user and the new agent is not automatically attached to the key's bound
workspace. Use a session JWT for the initial advanced deployment, then assign the agent explicitly if
workspace-scoped API access is required. See the [Remote Docker guide](/configuration/provisioner-backends/remote-docker)
for test scope, networking, capacity, sharing, and revocation limits.
...
| `403`  | API key lacks `agents:write`                                                                     |
