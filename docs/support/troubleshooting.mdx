...
<AccordionGroup>
  ...
  <Accordion title="Remote Docker issues">
    ...
    **Host is missing from the Deploy page**
    ...
    Confirm the host is enabled, configured, and its latest manual test succeeded. You must own it or
    hold an `editor`, `admin`, or `owner` role in a workspace where it is shared. Viewers see the host
    read-only but cannot deploy. The current dashboard injects Remote Docker targets into OpenClaw's
    picker only; Hermes Remote Docker uses the experimental
    [advanced API path](/configuration/provisioner-backends/remote-docker#hermes-advanced-api-path).

    ***

    **Agent or container still exists after unsharing the host**

    This is expected. Unsharing removes only the workspace grant. It does not stop, move, delete, or
    rebalance existing containers. After the agent owner loses their final qualifying grant, Nora
    ...
    **Remote host key changed after an intentional rebuild or rotation**
    ...
    out-of-band channel. Then open **App -> Remote Hosts**, choose **Reset SSH pin**, type the exact
    host label or id, and confirm. Nora clears the pin and previous Test state without changing the
    stored credentials. Run **Test** immediately; deployments and active use remain blocked until
    the new key is captured and pinned. If you cannot explain and independently verify the key
    change, do not reset it.
