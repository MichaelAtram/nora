...
- Session-only, user-owned Remote Docker registration plus workspace-based host grants
...
Shared agents and other workspace resources use workspace RBAC. Remote Docker hosts remain directly
user-owned and may be granted into a workspace without transferring their credentials or ownership.
A workspace has a creator (`workspaces.user_id`) plus a row per member in `workspace_members` with
one of four roles:
...
`NORA_WORKSPACE_INVITE_SECRET` (falls back to `JWT_SECRET`). For a shared Remote Docker host, viewers
can see the host and editors or higher can deploy; host configuration remains direct-owner-only.
...
- Remote-host SSH credentials are encrypted at rest and remain direct-owner-only. A workspace grant
  permits placement according to role but does not reveal the credential or transfer host ownership.
