...
A Remote Docker host has one direct user owner. From **App -> Remote Hosts**, that owner can share the
host into any workspace where they are a member. This is a compute-placement grant; it is separate
from assigning agents to a workspace.
...
Only the direct host owner manages its connection settings and encrypted credentials. Workspace
members never receive those secrets. Current remote-host grants are workspace-only: Nora does not
provide a platform-wide, user-group, organization-wide, or direct-user host grant. Platform admins
can inspect a read-only fleet inventory but do not automatically gain use or management access.
...
Unsharing a host removes only the `workspace -> remote host` grant. It does not stop, migrate, delete,
or rebalance existing containers or remove agent assignments. For an agent owner with no other
qualifying grant, Nora blocks new and queued deployment work, start/restart, live runtime and gateway
access, logs, terminal, environment updates, backup capture, non-stop scheduled actions, and ClawHub
operations. Live status and telemetry collection stops, and active Nora gateway, log, terminal, and
metrics streams are rechecked and closed. The stored agent record, lifecycle status, and previously
collected telemetry history remain visible to users who still have agent access, but they are no
longer live. **Stop**—manual or scheduled—and **Delete** remain available so the direct agent owner
or another authorized actor can clean up the residual workload.
...
Deleting a workspace removes the workspace, its agent memberships, and its Remote Docker host grants.
It does **not** delete agents or remote-host containers — they remain owned by their direct users.
Drain any workloads that rely on a host grant before deleting the workspace.
