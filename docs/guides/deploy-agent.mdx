...
<Step title="Enter a name">
  ...
  <Step title="Choose the execution target">
    ...
    A Remote Docker host appears only when its latest manual connection test succeeded and you own it
    or have an `editor`, `admin`, or `owner` workspace grant. Viewers can see a shared host but cannot
    select it. The current picker exposes Remote Docker only for OpenClaw; see the
    [Remote Docker guide](/configuration/provisioner-backends/remote-docker#hermes-advanced-api-path)
...
- [Remote Docker host setup and limitations](/configuration/provisioner-backends/remote-docker)
